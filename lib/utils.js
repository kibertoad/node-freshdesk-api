...
function handleResponse(error, response, body, request) {
		if (error) {
		...
		switch (response.statusCode) {
			...
			default:
				debug("path:[%s] raw body: %o", request.path);
				throw new FreshdeskError(body.description, body, response, request);
		}
}

// TODO: try to make less params here
async function makeRequest(method, auth, url, qs, data) {
	...
	if (data) {
		if ("attachments" in data && Array.isArray(data.attachments)) {
			...
			try {
				...
				return await handleResponse(null, response, responseBody, req);
			} catch (err) {
				cb(err);
			}
		} else {
	...
	try {
		...
		return await handleResponse(null, response, data, req);
	} catch (error) {
